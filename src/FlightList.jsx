...
const FlightList = () => {
  ...
  setTimeout(() => {
    ...
    if (flights[0]["returnDate"]) setReturnDate(flights[0]["returnDate"]);
  ...
  const handleRegBtn = async () => {
    ...
    try {
      const regResponce = await fetch(
        "https://fligthback.onrender.com/api/user/register",
        {
          method: "post",
          headers: {
  ...
  const handleLoginBtn = async (e) => {
    ...
    const loginRes = await fetch(
      "https://fligthback.onrender.com/api/user/login",
      {
        method: "post",
        headers: {
          "content-type": "application/json",
  ...
  const handleForget = async () => {
    try {
      ...
      const forgetFetch = await fetch(
        "https://fligthback.onrender.com/api/resetpassword",
        {
          method: "post",
          headers: {
            "content-type": "application/json",
  ...
  const fetchdata = async () => {
    try {
      ...
      const responce = await fetch(
        "https://fligthback.onrender.com/api/getuser",
        {
          method: "get",
          headers: {
            "content-type": "application/json",
      ...
      <header>
          ...
          <div className="logregfl mx-5">
            {loggedin ? (
              <i
                ...
                className="bx bx-user bx-lg usericonfl"
              ></i>
            ) : (
              <div className="logregbtn">
                <Button
                  className="mx-5 mb-1 mt-1"
                  variant="primary"
        ...
        <div className="flightlisthead ">
          ...
          <div className="date text-dark">
            {" "}
            {`Depart - ${departureDate.slice(8)} ____  Return - ${
              returnDate ? returnDate.slice(8) : "none"
            } `}{" "}
          </div>

          <div className="flightheadname  userflightlist">
            {" "}
                  ...
                  <div className="card-body row">
                    ...
                    <div className="flightdetails  row col-md-6 col-sm-12 text-center">
                      ...
                      <div className="departure departure2 row col-4 ">
                        <div className="hrs col-12">
                          {flight.flightDuration.slice(0, 1).concat("h")}
                        </div>
                        <div className="flightdir col-12">
                          <svg
                            className="svg"
                      ...
                      {flight.returnDate && (
                        <>
                          ...
                          <div className="arrival arrival1 col-4 ">
                            <div className="dflighttime flighttime">
                              {flight.departureTime.replace(
                                flight.departureTime[1],
                                +flight.departureTime[1] + 1
                              )}
                            </div>
                            <div className="airportcode">
                              {flight.arrivalAirportCode}
                          ...
                          <div className="arrival arrival3 row col-4 ">
                            <div className="aflighttime flighttime">
                              {flight.arrivalTime.replace(
                                flight.arrivalTime[1],
                                +flight.arrivalTime[1] + 1
                              )}
                            </div>
                            <div className="airportcode">
                              {flight.arrivalAirportCode}
                    ...
                    <div className="button col-md-3 col-sm-12  text-center">
                      <div className="price ">
                        {"₹ " + flight.price.slice(1) * passengercount}
                      </div>
                      <button
